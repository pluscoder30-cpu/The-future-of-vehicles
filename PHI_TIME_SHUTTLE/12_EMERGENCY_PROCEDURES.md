# Emergency Procedures — Temporal Fold Abort, Manual Override, and Evacuation
...
| Level 3 | Major anomaly (critical system failure) | Abort temporal fold, assess situation |
| Level 4 | Catastrophic (immediate danger) | Emergency abort, evacuate |

## 2. Temporal Fold Abort Procedures

### 2.1 Automatic Temporal Fold Abort

The temporal fold is aborted automatically when:
- Temporal fold bridge integrity < 95%
- Temporal fold field amplitude > 120% nominal
- Temporal fold frequency drift > 0.1%
- Temporal fold node position error > 0.001 seconds
- Metric coherence < 99%
- Temporal fold containment breach
- Passenger vital sign anomaly
- Structural integrity < 90%
- Temporal fold time > 2× nominal
- CTC risk detected
- Paradox risk detected

**Automatic temporal fold abort sequence:**
```
Time 0 ms: Abort logic triggers
Time 10 ms: Power cut to all temporal coils
Time 60 ms: Temporal fold field begins collapsing
Time 560 ms: Temporal fold bridge dissolving
Time 1650 ms: Temporal metric verified flat
Time 2000 ms: Vehicle time verified, systems checked
```

**Total abort time: 2 seconds**

### 2.2 Manual Temporal Fold Abort

The crew can initiate a manual abort at any time:

**Manual temporal fold abort procedure:**
```
1. Press red temporal fold abort button (pilot or copilot console)
2. Verify temporal fold abort initiated (temporal status indicator: ABORT)
3. Monitor temporal fold collapse (temporal status indicator: COLLAPSING)
4. Verify vehicle time (atomic clock)
5. Verify temporal metric flat (coherence sensor)
6. Assess situation (determine cause of abort)
7. Decide: retry temporal fold or return to base
8. Log abort event (data recorder)
9. Notify ground control (if communication available)
```

### 2.3 Temporal Fold Abort Recovery

After temporal fold abort, the vehicle:
- Is at its original time (within ±0.001 seconds)
- Has all systems operational (temporal coils may need cooldown)
- Can attempt another temporal fold after a 30-second cooldown
- Must wait 5 minutes if the abort was due to temporal fold containment breach

**Recovery checklist:**
```
□ Verify vehicle time (atomic clock)
□ Verify temporal metric flat (coherence sensor)
□ Check temporal coil status (temperature, integrity)
□ Check battery status (charge level, temperature)
□ Check navigation system (target time lock, accuracy)
□ Check communication system (radio, temporal-linked)
...
□ Decide: retry temporal fold or return to base
```

## 3. Manual Override Procedures

### 3.1 Temporal System Override

The crew can override temporal system operations:

| Override | Button | Effect |
|----------|--------|--------|
| Temporal fold abort | Red button (pilot/copilot) | Immediate temporal fold abort |
| Temporal fold initiation | Green button (pilot only) | Start temporal fold sequence (requires copilot confirmation) |
| Temporal fold hold | Yellow button (pilot/copilot) | Pause temporal fold sequence at current phase |
| Temporal fold resume | Green button (pilot/copilot) | Resume temporal fold sequence from hold |
| Emergency quench | Red button + twist (pilot only) | Emergency temporal fold quench (all energy to heat sinks) |
...
| Override | Button | Effect |
|----------|--------|--------|
| Target time change | Navigation keypad | Change target time |
| Target time cancel | Red button (navigation) | Cancel current target time |
| Manual alignment | Joystick (copilot) | Manually align temporal fold bridge |
| Emergency return | Red button (pilot) + voice command | Emergency return to base time |
...
| Fire in temporal coil bay | Yes | Emergency exit, slide |
| Temporal fold containment breach | No (stay in vehicle) | Temporal fold abort, wait for metric relaxation |
| Structural damage | Yes (if safe) | Emergency exit, slide |
| Life support failure | No (use emergency supply) | Temporal fold abort, return to base time |
| Passenger medical emergency | No (treat in vehicle) | Temporal fold abort, return to base time |
...
Emergency evacuation procedure:
  ...
  2. Initiate temporal fold abort (if in temporal fold)
  3. Verify vehicle stationary and temporal metric flat
  4. Shut down temporal coils and batteries
...
| Total evacuation time (2 passengers) | 20 seconds |
...
Emergency communication procedure:
  ...
  4. Use temporal-linked communication (if available)
...
Emergency message format:
  ...
  Nature of emergency: [temporal fold failure / fire / structural damage / medical]
...
Medical emergency procedure:
  ...
  3. Use temporal cocoon vital sign monitoring
  4. Administer medication (if available and trained)
  5. Initiate temporal fold abort (if in temporal fold)
  6. Return to base time (nearest hospital)
...
|-----------|--------|----------|
| Cardiac arrest | CPR + AED, temporal fold abort, return to base | Critical |
| Severe bleeding | Tourniquet, temporal fold abort, return to base | Critical |
| Fracture | Splint, temporal fold abort, return to base | High |
| Burns | Cool, cover, temporal fold abort, return to base | High |
| Allergic reaction | Epinephrine, temporal fold abort, return to base | High |
| Head injury | Monitor, temporal fold abort, return to base | High |
| Seizure | Protect, monitor, temporal fold abort, return to base | High |
...
Post-emergency checklist:
  ...
  □ Vehicle secured (temporal coils off, batteries isolated)
...
1. Secure the vehicle (temporal coils off, batteries isolated)
